...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        # 1. Instantiate the multi-headed self-attention mechanism
        self.attention = self.MultiHeadedSelfAttention(model_dim, num_heads)
        
        # 2. Instantiate the feed-forward neural network
        self.feed_forward = self.VanillaNeuralNetwork(model_dim)
        
        # 3. Instantiate two LayerNorm layers to apply Pre-LN (before attention and before feed-forward)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        # First residual connection with Pre-LN applied to the input before attention
        x = embedded + self.attention(self.ln_1(embedded))
        
        # Second residual connection with Pre-LN applied to the intermediate state before the FFN
        x = x + self.feed_forward(self.ln_2(x))
        
        # Return the final tensor rounded to 4 decimal places
